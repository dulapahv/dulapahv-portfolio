...
const a = (props: HTMLProps<HTMLAnchorElement>) => {
  ...
  return <Link className="text-mirai-red hover:underline" {...props} />;
};
...
const img = (properties: HTMLProps<HTMLImageElement>) => {
  ...
  return (
    <Image
      ...
      className="border-border my-4 overflow-hidden rounded-md border"
      quality={100}
    />
  );
};
      ...
      <HeadingTag id={id} {...props} className="group relative">
        {id && (
          <Link
            href={`#${id}`}
            className={cn(
              'absolute top-1/2 -left-6 -translate-y-1/2 opacity-0 group-hover:opacity-100',
              level === 'h2' && 'pt-10',
            )}
