...
import { contributionsData, getContributionsByYear, getContributionStats } from './contributions';
...
describe('Contributions Module', () => {
  describe('contributionsData', () => {
    ...
    it('should have valid contribution statuses', () => {
      contributionsData.forEach(contribution => {
        expect(['MERGED', 'OPEN', 'CLOSED', 'DRAFT']).toContain(contribution.status);
      });
    });
  });
  ...
  describe('getContributionsByYear', () => {
    ...
    it('should have date instances for each contribution', () => {
      const result = getContributionsByYear();
      Object.values(result).forEach(yearContributions => {
  ...
  describe('getContributionStats', () => {
    ...
    it('should count merged contributions', () => {
      const stats = getContributionStats();
      const expectedMerged = contributionsData.filter(c => c.status === 'MERGED').length;
    ...
    it('should count open contributions', () => {
      const stats = getContributionStats();
      const expectedOpen = contributionsData.filter(c => c.status === 'OPEN').length;
    ...
    it('should count closed contributions', () => {
      const stats = getContributionStats();
      const expectedClosed = contributionsData.filter(c => c.status === 'CLOSED').length;
    ...
    it('should have stats that sum to total', () => {
      ...
      expect(stats.merged + stats.open + stats.closed).toBeLessThanOrEqual(stats.total);
    });
  });
});
