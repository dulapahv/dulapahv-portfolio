...
interface JsdocVarsOptions {
  variables?: Record<string, string>;
  pattern?: RegExp;
}

function createBeforeWriteFile(
  options: JsdocVarsOptions = {}
...
export default defineConfig({
  plugins: [
    dts({
      beforeWriteFile: createBeforeWriteFile()
    })
  ]
});
```
...
1. **Define Variables**: In your `package.json`, define the variables you want to use in your JSDoc comments under the `config.jsdoc` section. You can also define them directly in the `createBeforeWriteFile` options if preferred. (see [Options and Customization](#options-and-customization) below)
...
> [!Important]
> The variables should be wrapped with `$` symbols. This is chosen to avoid conflicts with other JSDoc syntax. You can customize the pattern in the `createBeforeWriteFile` options if needed. (see [Options and Customization](#options-and-customization) below)
...
The `createBeforeWriteFile` function accepts an options object that allows you to customize its behavior:
...
```ts
dts({
  beforeWriteFile: createBeforeWriteFile({ variables: pkg.config?.jsdoc || {} })
});
```
