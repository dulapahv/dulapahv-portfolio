...
Use the `&` with `>` and the tag name to target direct children by their tag name.
...
Use the `&` with `>` and the class name to target direct children by their class.
...
Use the `&` with `>` and the data attribute wrapped in square brackets to target direct children by their data attributes.
